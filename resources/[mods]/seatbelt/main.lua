...
Fwv = function (entity)
    ...
    if car ~= 0 and (wasInCar or IsCar(car)) then
      ...
      if IsControlJustReleased(0, 20) and GetLastInputMethod(0) then
        beltOn = not beltOn 
        if beltOn then 
		  TriggerEvent("pNotify:SendNotification", {text = "Cinturón puesto", type = "success", timeout = 1400, layout = "centerLeft"})
		  ...
		  TriggerEvent("pNotify:SendNotification", {text = "Cinturón quitado", type = "error", timeout = 1400, layout = "centerLeft"})
