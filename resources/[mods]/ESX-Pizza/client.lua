...
local coefflouze = 0.4 --Coeficient multiplicateur qui en fonction de la distance definit la paie
  ...
  while true do
    ...
    if isInJobPizz == false then
      ...
      if GetDistanceBetweenCoords(pizzeria.x, pizzeria.y, pizzeria.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then
        HelpText("Pulsa ~INPUT_CONTEXT~ para iniciar la entrega de ~r~pizza",0,1,0.5,0.8,0.6,255,255,255,255)
    ...
    if isToHouse == true then
      ...
      while notif == true do
        ...
        TriggerEvent("pNotify:SendNotification", {
          text = "Dirección : " ..destinol.. " para entregar pizza",
          type = "info",
          queue = "global",
          timeout = 4000,
          layout = "bottomRight"
      ...
      if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 3 then
        ...
        if IsControlJustPressed(1,38) then
          ...
          if (posibilidad > 70) and (posibilidad < 90) then
            ...
            TriggerEvent("pNotify:SendNotification", {
              text = "Una pequeño incentivo : " .. pourboire .. "€",
              type = "success",
              queue = "global",
              timeout = 4000,
              layout = "bottomRight"
    ...
    if isToPizzaria == true then
      ...
      if GetDistanceBetweenCoords(pizzeria.x,pizzeria.y,pizzeria.z, GetEntityCoords(GetPlayerPed(-1),true)) < 3 and afaitunepizzamin == true then
        HelpText("Pulsa ~INPUT_CONTEXT~ para recuperar las pizzas",0,1,0.5,0.8,0.6,255,255,255,255)
        ...
        if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("faggio2"))  then
          ...
          if IsControlJustPressed(1,38) then
            ...
            if IsInVehicle() then
              ...
              TriggerEvent("pNotify:SendNotification", {
                text = "Gracias por su trabajo, aquí está su dinero : " .. paie .. "€",
                type = "success",
                queue = "global",
                timeout = 4000,
                layout = "bottomRight"
  ...
  while true do
    ...
    if isInJobPizz == true then
      ...
      if GetDistanceBetweenCoords(pizzeriafin.x, pizzeriafin.y, pizzeriafin.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then
        HelpText("Pulsa ~INPUT_CONTEXT~ para repartir la ~r~pizza",0,1,0.5,0.8,0.6,255,255,255,255)
        ...
        if IsControlJustPressed(1,38) then
          ...
          if afaitunepizzamin == true then
            ...
            TriggerEvent("pNotify:SendNotification", {
              text = "Gracias por haber trabajado, buen día.",
              type = "success",
              queue = "global",
              timeout = 4000,
              layout = "bottomRight"
