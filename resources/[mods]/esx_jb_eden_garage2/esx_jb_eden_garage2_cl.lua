...
function ListVehiclesMenu(garage, KindOfVehicle)
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		...
		function(data, menu)
			local elem = {}
			table.insert(elem, {label ="Retirar vehículo" , value = 'get_vehicle_out'})
			table.insert(elem, {label ="Renombrar vehículo" , value = 'rename_vehicle'})
				...
				function(data2, menu2)
					if data2.current.value == "get_vehicle_out" then
                        if (data.current.value.fourrieremecano) then
                            TriggerEvent('esx:showNotification', 'Tu vehículo está en el mecánico')
                        elseif (data.current.value.state) then
                            menu.close()
                            menu2.close()
                            SpawnVehicle(data.current.value.vehicle, garage, KindOfVehicle)
                        else
                            TriggerEvent('esx:showNotification', 'Tu vehículo ya está fuera')
                        end
...
function StockVehicleMenu(KindOfVehicle)
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		if GetPedInVehicleSeat(vehicle, -1) == playerPed then
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'Su trailer está en el garaje.')
					else
						TriggerEvent('esx:showNotification', 'No puedes guardar este vehículo.')
					end
			...
			TriggerEvent('esx:showNotification', 'Usted no es conductor del vehículo.')
		end
	else
...
function StockVehicleFourriereMenu()
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		TriggerEvent('esx:showNotification', 'No hay un vehículo cerca')
	end
end
	...
	ESX.TriggerServerCallback('eden_garage:getOutVehicles', function(vehicles)
		...
		if not table.empty(vehicles) then
			for _,v in pairs(vehicles) do
				...
				if v.fourrieremecano then
					labelvehicle = vehicleName..': Alimentación externa'
					table.insert(elements, {label =labelvehicle , value = 'fourrieremecano'})
				else
					labelvehicle = vehicleName..': Retirar'
