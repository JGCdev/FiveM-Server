...
function OpenPropertyMenu(property)
	local elements = {}
	local priceAlquiler = math.floor(property.price / 200)
	if PropertyIsOwned(property) then
		...
		if not Config.EnablePlayerManagement then
			table.insert(elements, {label = _U('buy') .. ' <span style="color:green;">' .. property.price .. '€</span>', value = 'buy'})
			table.insert(elements, {label = _U('rent') .. ' <span style="color:green;">' .. priceAlquiler .. '€</span>', value = 'rent'})
		end
...
function OpenGatewayAvailablePropertiesMenu(property)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_available_properties', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_available_properties_actions', {
			...
			align    = 'top-left',
			priceAlquiler = property.price / 200,
			elements = {
				{label = _U('buy') .. property.price .. '€', value = 'buy'},
				{label = _U('rent') .. priceAlquiler .. '€', value = 'rent'},
