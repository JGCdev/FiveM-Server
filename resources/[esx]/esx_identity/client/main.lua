...
RegisterNUICallback('escape', function(data, cb)
	if hasIdentity then
		...
		TriggerEvent('chat:addMessage', { args = { '^1[IDENTITY]', '^1Debes crear un personaje para poder jugar' } })
	end
end)
...
function verifyName(name)
	...
	if count ~= nameLength then
		return 'Tu nombre no puede contener carácteres especiales.'
	end
	...
	if spacesInName > 2 then
		return 'Tu nombre contiene más de dos espacios'
	end
