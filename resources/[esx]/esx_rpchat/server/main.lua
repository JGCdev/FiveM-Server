...
AddEventHandler('chatMessage', function(source, name, message)
	if string.sub(message, 1, string.len('/')) ~= '/' then
		...
		TriggerClientEvent('chat:addMessage', -1, { args = { _U('ooc_prefix', name), message }, color = { 201, 201, 201 } })
	end
end)
...
RegisterCommand('anontwt', function(source, args, rawCommand)
	...
	TriggerClientEvent('chat:addMessage', -1, { args = { _U('twt_prefix_ano', name), args }, color = { 0, 153, 204 } })
	--print(('%s: %s'):format(name, args))
end, false)
